...
- 默认内置 `ALLOWED_TARGETS=http://127.0.0.1:9090`
- 初始配置界面可自动读取 `ALLOWED_TARGETS` 并预填控制器地址
...
面板会通过当前仓库的 GitHub Releases 检查前端更新。

```text
https://api.github.com/repos/YTjungle666/YT-ClashBoard/releases/latest
```

发布新版本时，建议保持 Git tag / Release 与仓库版本一致，以便面板正确识别更新。
...
|---|---:|---|
| `PORT` | `80` | 服务端监听端口 |
| `ALLOWED_TARGETS` | `http://127.0.0.1:9090` | 允许服务端代理访问的控制器 base URL 列表，多个值用英文逗号分隔 |
...
所以直接拉取镜像启动时，即使你不手动补这条环境变量，也会先有一个默认值。

如果你用容器或面板部署，除了 `PORT`，还可以按需覆盖：
